...
import { LogLevel, LoggerNest, LoggerNestInterface } from './logger.nest';

export type { LoggerNestInterface, LogLevel } from './logger.nest';

export abstract class LoggerNestClass implements LoggerNestInterface {
  ...
  private static readonly LOGGER_METADATA_OPTIONS: LoggerMetadataOutputOptionsInterface = {
    hideMethodSet: LoggerNestClass.LOGGER_METHOD_SET,
  };
  private readonly logger: LoggerNest;

  public constructor(options: LoggerOptionsInterface) {
    this.logger = new LoggerNest(options);
  }
  ...
  protected stdout(
    ...
    this.logger.print(level, origin, [message, ...params], {
      ...LoggerNestClass.LOGGER_METADATA_OPTIONS,
